    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg m-7 p-10 items-center justify-center">
                <p class="md:text-7xl sm:text-4xl text-2xl uppercase font-bold font-serif text-blue-700">Sub Caregory
                </p>
                <div class="my-7 py-10 items-center justify-center">
                    ...
                    " class="w-full " enctype="multipart/form-data">
                        ...
                        <div class="flex flex-wrap -mx-3 mb-6">
                            ...
                            <div class="w-full md:w-1/2 px-3 py-5 md:mb-0">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                                    for="main_categories_id">
                                    Select Main Category
                                </label>
                                @isset($mainCategorys)
                                    <select
                                        class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                                        name="main_categories_id" id="main_categories_id">
                                        @foreach ($mainCategorys as $mainCategory)
                                            <option value="{{ $mainCategory->id }}">
                                                Name : {{ $mainCategory->name_en }}
                                            </option>
                                        @endforeach
                                    </select>
                                @else
                                    <p class="md:text-2xl sm:text-xl m-3 text-lg uppercase font-serif text-black">add main
                                        category to
                                        show
                                    </p>
                                @endisset
